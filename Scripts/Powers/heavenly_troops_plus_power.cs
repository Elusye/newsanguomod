...
/// <summary>
/// “天降雄兵+”：由升级版“天降雄兵”获得。同一回合内打出的天降雄兵+会叠加到同一实例
/// （士兵数量累加），不同回合打出的各自独立倒计时。
/// 每个实例：Amount 表示累计士兵数量，倒计时固定为 2，经过 2 次玩家回合结束（弃牌后）
/// 时将 Amount 张“士兵+”加入手牌并移除。
/// 图标角标：右上角显示士兵数量，右下角显示剩余回合数。
/// </summary>
[RegisterPower]
public class heavenly_troops_plus_power : ModPowerTemplate, IPowerExtraIconAmountLabelSpecsProvider, IPowerExtraIconAmountLabelsChangeSource
{
